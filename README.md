...
> **Zero-config iOS Safari chrome染色.** Paints the status bar and URL bar to match your page content at each viewport edge — gradients, sections, rubber-band overscroll, all handled automatically. One import. No theme-color juggling. No tint configuration. It just works.
...
iOS Safari染色 the chrome (status bar + URL bar) with **whatever happens to be at the viewport edge** — but the rules are quirky, undocumented, and shift between iOS versions. You ship a page with a gradient hero, and it looks great… until:

- The viewport bottom染色 mint while your belt section is dark teal — a visible **seam** at the chrome boundary.
- Compact tab bar appears, the染色 "shifts" by 30px because `100lvh - 100svh` doesn't match the current chrome height.
...
- `position: fixed` elements染色 chrome correctly… except when they don't, depending on `opacity`, `display`, viewport edge proximity, and dark-mode mood.
...
- **Top chrome染色** stays light and unobtrusive (Safari's natural sampling of whatever's at viewport top).
- **Bottom chrome染色** mirrors the page content: gradient interp when you're in gradient territory, section color when an opaque section reaches the edge, footer color when you're at page-end.
- **Overscroll染色** when the page-end section enters viewport — `bleed` overwrites `<html>`, `<body>`, AND `body::before` so the rubber-band exposed area染色 the same color, not your fallback bg.
...
If you have your own sticky header or footer banner that already染色 chrome correctly, mark it with `.bleed-top` / `.bleed-bottom`. `bleed` will detect it and **step out of the way** on that edge.
...
| `SAFE_NATURAL` | Page content at viewport edge already produces the right chrome染色 (e.g. top edge, mid-page section) | Hides the bleed tint (`display:none`) so Safari染色 chrome with its native edge sampling. |
| `BLEED_OVERRIDE` | Page content at viewport edge would染色 the wrong color (e.g. gradient terminal ≠ html bg, or page-end section needs explicit染色) | Renders a 12px tint at the edge with the correct color. Safari samples it for chrome染色. |

For the bottom edge specifically:

- **Gradient territory**: extend the gradient interpolation into the chrome.
- **Mid-page section** (e.g. a belt between gradient and footer): 退場 — let Safari's edge sampling render the natural translucent chrome.
- **Last section** (footer at page-end): 進場染色 the section color. Also overwrites `<html>`, `<body>`, and `body::before` so iOS rubber-band overscroll染色 the same color and you don't see the html-bg mint leak through.
...
- **`opacity: 0` is still sampled** — to truly "退場", you need `display: none`.
...
| Browser | Behavior |
|---|---|
| **iOS Safari 26+** | Full染色 with native chrome sampling. Tested extensively. |
| **iOS Safari 15-25** | Falls back to `theme-color` meta染色 (we update it in sync) plus tint rendering. |
| **Desktop Safari, Chrome, Firefox** | No-op. Tint elements are kept `display: none` on non-iOS to avoid a colored band on browsers that don't染色 chrome. |
